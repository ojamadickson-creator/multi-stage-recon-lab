...
We built a realistic cybersecurity lab to understand how attackers reconnoiter, compromise, and move through Windows Active Directory environments — and how security teams can detect these activities using Splunk SIEM.

Think of it like this: we put on both hats. First, the red team hat, where we mapped the network, found services, and eventually got valid credentials. Then we switched to the blue team hat, building detection rules and dashboards to catch exactly the kinds of activities we'd just performed.

## What We Found

### The Attack Worked (And That's Expected)

Here's the thing — in our lab environment, a motivated attacker with network access could:
...
Our Splunk dashboard successfully visualized the entire attack chain:
...
SIEM sees the network perspective; EDR sees the endpoint perspective. Together, they're far more effective.
...
The techniques demonstrated in this lab aren't advanced nation-state tactics — they're fundamental, well-documented methods that any competent attacker can employ with freely available tools. The difference between a breach and a blocked attack often comes down to:

1. **Whether you're collecting the right logs** (we are)
2. **Whether someone is watching those logs** (we built the dashboards)
3. **Whether you respond quickly enough** (our alerts fire in under a minute)

**Bottom line:** The defenses work, but only if they're maintained, monitored, and continuously improved.
