...
### Phase 1: Network Reconnaissance (T+0 to T+5 minutes)
...
### Phase 2: LDAP Enumeration (T+5 to T+10 minutes)
...
### Phase 3: Password Spraying (T+10 to T+15 minutes)

**Tools Used:** NetExec (nxc) v1.x

**Commands Executed:**
```bash
nxc smb 192.168.56.102 -u vagrant -p /tmp/passwordlist
```
...
### Phase 4: Post-Compromise Enumeration (T+15 to T+20 minutes)
...
**Results:**
- Query returned 1,205 failed logon events
- Source IP 192.168.57.10 (Kali) generated 1,191 failed attempts
...
```spl
index=main sourcetype="WinEventLog" EventCode=4624 src_ip="192.168.57.10"
```
...
```spl
index=main host=dc (EventCode=4624 OR EventCode=4625 OR EventCode=4672) src_ip="192.168.57.10"
| eval attack_phase=case(...)
```

**Results:**
- Successfully correlated multi-phase attack
- Identified progression from brute force → successful auth → privilege escalation
...
| Privilege Escalation | Event 4672 from attacker IP | ✅ Detected | Confirmed admin logon |
...
| 2 | No account lockout threshold | HIGH | 1,191 failed attempts allowed |
...
| NetExec (nxc) | 1.x | SMB enumeration, password spraying, command execution |
