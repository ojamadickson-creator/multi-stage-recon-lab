...
fd = [["Finding","Severity","Evidence"],
    ...
    ["SMB accessible from attacker","MEDIUM","Hydra/NetExec successful via SMB"],
...
mt.setStyle(TableStyle([('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('FONTSIZE',(0,0),(-1,-1),9),
    ('LINEABOVE',(0,0),(-1,0),1.5,black),('LINEBELOW',(0,0),(-1,0),0.75,black),
    ('LINEBELOW',(0,-1),(-1,-1),1.5,black),('TOPPADDING',(0,0),(-1,-1),6),('BOTTOMPADDING',(0,0),(-1,-1),6),
    ('VALIGN',(0,0),(-1,-1),'TOP')]))
story.append(mt)
...
story.append(Paragraph("4.1 Phase 0: Username Acquisition via Social Engineering", H2))
...
story.append(Paragraph("4.2 Phase 1: Network Reconnaissance", H2))
...
story.append(Paragraph("4.3 Phase 2: LDAP Enumeration", H2))
...
story.append(Paragraph("4.4 Phase 3: Credential Access via Password Spraying", H2))
...
story.append(Paragraph("Step 2 — Execute Password Spray with Hydra:", H3))
...
story.append(Paragraph("Screenshot 4: Hydra password spray with password list verification.", CAP))

story.append(Paragraph("4.5 Phase 4: Post-Compromise Enumeration", H2))
...
story.append(Paragraph("Screenshot 5: Post-compromise privilege enumeration.", CAP))
story.append(PageBreak())
...
story.append(Paragraph("Screenshot 6: Splunk server CLI showing service status.", CAP))
...
story.append(Paragraph("Screenshot 7: Brute force detection showing 105 failed attempts from 192.168.57.10.", CAP))
...
story.append(Paragraph("Screenshot 8: Successful authentication query showing attacker IP 192.168.57.10.", CAP))
...
story.append(Paragraph("Screenshot 9: Time series query for visualizing attack patterns.", CAP))
...
story.append(Paragraph("Screenshot 10: Detection Engineering1 dashboard with failed logins, time series, and successful login panels.", CAP))
add_screenshot("Detect-Dashboard-Timeseries.png", 15*cm)
story.append(Paragraph("Screenshot 11: Saving the Time Series Visualization panel to the Detection Engineering1 dashboard.", CAP))
...
story.append(Paragraph("Screenshot 12: Brute Force alert configuration in Splunk.", CAP))
add_screenshot("Alert-Result.png", 15*cm)
story.append(Paragraph("Screenshot 13: Alert trigger history showing fired alerts.", CAP))
...
story.append(Paragraph("Screenshot 14: Correlation query mapping attack phases to specific EventCodes.", CAP))
story.append(PageBreak())
...
story.append(Paragraph("Screenshot 15: Wireshark capture showing SYN scan packets.", CAP))
add_screenshot("ldap packets.png", 15*cm)
story.append(Paragraph("Screenshot 16: Wireshark capture showing LDAP query packets.", CAP))
